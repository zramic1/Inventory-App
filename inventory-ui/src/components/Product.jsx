...
import { getAllProductsForUser, setShowNotificationForLowQuantity, getUniqueProductsForUser, getAllWarehouses } from "./actions/loginActions";
...
function Product() {
  ...
  const [filterDropdownText, setFilterDropdownText] = useState("All warehouses");

  const getProducts = () => {
    let url = productContext.product;
    let parsirajIdWarehousa = parseInt(filterDropdownText.replace(/[^0-9]/g, ''));
    let putanja = filterDropdownText === "All warehouses" ? `/products/user/${trenutniKorisnikId}` : `/products/warehouse/${parsirajIdWarehousa}`
    axiosInstance(url)
      .get(putanja)
      .then((res) => {
        console.log(
